...
def load_metrics():
    ...
    try:
        with open("data/attention_model_metrics.json") as f:
...
def load_sample_listings():
    ...
    21k-row dataset) at data/sample_listings.csv with at least:
    lat, long, price, bedrooms, bathrooms, sqft_living, zipcode
    """
    path = "data/sample_listings.csv"
    if os.path.exists(path):
    ...
    if submitted:
        if model is None:
            ...
            if sample_df is not None:
                ...
                st.info(
                    "Add `data/sample_listings.csv` to your repo to show nearby comparable "
...
with tab3:
    ...
    if sample_df is not None and {"lat", "long", "price"}.issubset(sample_df.columns):
        ...
        st.info(
            "Add `data/sample_listings.csv` (with lat, long, price, zipcode columns) "
            "to your repo to power the map and zipcode breakdown here."
        )

    err_path = "data/zipcode_error_summary.csv"
